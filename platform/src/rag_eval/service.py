...
class PlatformService:
    def __init__(self, paths: PlatformPaths, *, product_enabled: bool | None = None) -> None:
        ...
        if self.portfolios is not None:
            ...
            # an Authoring Case/Gold or any executable Benchmark payload.
    ...
    def admit_new_public_experiment(
        ...
        return self.resolved_run_plans.create(
            self.resolve_new_public_experiment(experiment)
        )
    ...
    def resolve_new_public_experiment(
        ...
        if self.formal_datasets is None:
            raise NewRunAdmissionError(
                "immutable Benchmark Releases are unavailable in this Platform mode"
            )
        try:
            benchmark = self.formal_datasets.resolve_native_benchmark(
                experiment.dataset_release_id,
                case_ids=(
                    None
                    if experiment.case_ids is None
                    else tuple(experiment.case_ids)
                ),
            ...
            raise NewRunAdmissionError(
                "selected Benchmark Release cannot produce a verified native snapshot"
            ) from exc
        try:
    ...
    def queue_new_public_experiment(
        ...
        reference = self.admit_new_public_experiment(experiment)
