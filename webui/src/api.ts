...
export const api = {
  ...
  caseIndex: (runId: string) => request<CaseListItem[]>(`/runs/${encodeURIComponent(runId)}/cases/index`),
  case: (runId: string, caseId: string, repetition: number) => request<CaseResult>(`/runs/${encodeURIComponent(runId)}/cases/${encodeURIComponent(caseId)}?repetition=${encodeURIComponent(repetition)}`),
